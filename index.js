...
function updateOrder() {
  ...
  cinema.options.orderBlock.innerHTML = renderOrder();
  ...
  function renderOrder() {
    return `<div class='order-text'>You chose places:</div>
    <div class='order-places'>
			${renderOrderPlaces(cinema.order)}
    	<div class='order-total'>Total:${calcOrderTotal(cinema.order)}</div>
    </div>
    <button class='order-buy'>Get Tickets</button>
    <button class='order-cancel'>Cancel</button>`;
  }
  function renderOrderPlaces(places) {
    let placesHtml = "";
    places.forEach(p => {
      placesHtml += `<div class='order-place'>Row ${p.row} position ${
        p.position
      }</div>`;
    });
    return placesHtml;
